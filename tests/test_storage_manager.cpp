...
TEST_F(StorageManagerTest, ExtendedAPIsValidation) 
{
    ...
    EXPECT_EQ(sm->saveFamilyDataEx(invalid_family, &family_id), commons::Result::InvalidInput);

    // Test valid family save with ID return
    Family valid_family("Test Family");
    EXPECT_EQ(sm->saveFamilyDataEx(valid_family, &family_id), commons::Result::Ok);
    EXPECT_GT(family_id, 0);

    // Test member save validation
    Member invalid_member("", "");  // Empty name
    uint64_t member_id = 0;
    EXPECT_EQ(sm->saveMemberDataEx(invalid_member, family_id, &member_id), commons::Result::InvalidInput);

    // Test member save with non-existent family
    Member valid_member("John", "Johnny");
    EXPECT_EQ(sm->saveMemberDataEx(valid_member, 999, &member_id), commons::Result::NotFound);

    // Test valid member save with ID return
    EXPECT_EQ(sm->saveMemberDataEx(valid_member, family_id, &member_id), commons::Result::Ok);
...
TEST_F(StorageManagerTest, ExtendedAPIsUpdateOperations) 
{
    ...
    uint64_t family_id = 0;
    ASSERT_EQ(sm->saveFamilyDataEx(family, &family_id), commons::Result::Ok);

    Member member("Original Name", "Original Nick");
    uint64_t member_id = 0;
    ASSERT_EQ(sm->saveMemberDataEx(member, family_id, &member_id), commons::Result::Ok);

    // Test family update validations
    EXPECT_EQ(sm->updateFamilyDataEx(family_id, ""), commons::Result::InvalidInput);
    EXPECT_EQ(sm->updateFamilyDataEx(999, "New Name"), commons::Result::NotFound);
    EXPECT_EQ(sm->updateFamilyDataEx(family_id, "Updated Family"), commons::Result::Ok);
    ...
    EXPECT_EQ(sm->updateMemberDataEx(999, "New Name", "New Nick"), commons::Result::NotFound);
    EXPECT_EQ(sm->updateMemberDataEx(member_id, "Updated Name", "Updated Nick"), commons::Result::Ok);
    ...
    EXPECT_EQ(sm->updateMemberDataEx(member_id, "", "New Nickname Only"), commons::Result::Ok);
    ...
    // Update only name
    EXPECT_EQ(sm->updateMemberDataEx(member_id, "New Name Only", ""), commons::Result::Ok);
    updated_member.reset(sm->getMemberData(member_id));
    ASSERT_NE(updated_member, nullptr);
    ...
    EXPECT_EQ(sm->updateMemberDataEx(member_id, "", ""), commons::Result::InvalidInput);
}
...
TEST_F(StorageManagerTest, ExtendedAPIsDeleteOperations) 
{
    ...
    ASSERT_EQ(sm->saveMemberDataEx(member1, family_id, &member1_id), commons::Result::Ok);
    ASSERT_EQ(sm->saveMemberDataEx(member2, family_id, &member2_id), commons::Result::Ok);

    // Test member deletion
    EXPECT_EQ(sm->deleteMemberDataEx(999), commons::Result::NotFound);
    EXPECT_EQ(sm->deleteMemberDataEx(member1_id), commons::Result::Ok);
    EXPECT_EQ(getTableRowCount("MemberInfo"), 1);  // One member remains

    // Test cascade delete via family deletion
    EXPECT_EQ(sm->deleteFamilyDataEx(999), commons::Result::NotFound);
    EXPECT_EQ(sm->deleteFamilyDataEx(family_id), commons::Result::Ok);
    EXPECT_EQ(getTableRowCount("FamilyInfo"), 0);
